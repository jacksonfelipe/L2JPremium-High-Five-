...
public class VoteRewardHopzone
{
	...
	private static int getFirstPageRankVotes()
	{
		...
		try
		{
			if (page1Url == null || page1Url.trim().isEmpty() || (!page1Url.startsWith("http://") && !page1Url.startsWith("https://")))
			{
				return -1;
			}
		...
		{
			_log.warn("Hopzone: Error while getting server vote count: " + e.getMessage());
		}
	...
	private static int getVotes()
	{
		...
		try
		{
			if (hopzoneUrl == null || hopzoneUrl.trim().isEmpty() || (!hopzoneUrl.startsWith("http://") && !hopzoneUrl.startsWith("https://")))
			{
				return -1;
			}
