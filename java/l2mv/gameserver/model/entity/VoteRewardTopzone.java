...
public class VoteRewardTopzone
{
	...
	private static void reward()
	{
		...
		{
			/*
			 * if (firstPageVotes == -1) { _log.info("Topzone: There was a problem on getting votes from server with rank " + firstPageRankNeeded + "."); }
			 */
			if (currentVotes == -1)
			{
		...
		if (lastVotes == 0)
		{
			...
			/*
			 * if ((firstPageVotes - lastVotes) <= 0) { Announcements.getInstance().announceToAll("Topzone: We are in the first page of topzone, so the reward will be big."); if (Config.ALLOW_TOPZONE_GAME_SERVER_REPORT) { _log.info("Server is on the first page of topzone."); } } else {
			 * Announcements.getInstance().announceToAll("Topzone: We need " + (firstPageVotes - lastVotes) + " vote(s) to get to the first page of topzone for big reward."); if (Config.ALLOW_TOPZONE_GAME_SERVER_REPORT) { _log.info("Server votes needed for first page: " + (firstPageVotes -
			 * lastVotes)); } }
			 */
			return;
		}
		...
		if (currentVotes >= (lastVotes + voteRewardVotesDifference))
		{
			...
			playerIps.clear();
			/*
			 * else { if (Config.ALLOW_TOPZONE_GAME_SERVER_REPORT) { _log.info("Server votes on topzone: " + currentVotes); _log.info("Server votes needed for first page: " + (firstPageVotes - lastVotes)); _log.info("Votes needed for next reward: " + ((currentVotes + voteRewardVotesDifference) -
			 * currentVotes)); } Announcements.getInstance().announceToAll("Topzone: Everyone has been rewarded with small reward."); Announcements.getInstance().announceToAll("Topzone: Current vote count is " + currentVotes + "."); Announcements.getInstance().announceToAll("Topzone: We need " +
			 * (firstPageVotes - currentVotes) + " vote(s) to get to the first page of topzone for big reward."); for (Player player : GameObjectsStorage.getAllPlayers()) { boolean canReward = false; String pIp = player.getIP(); if (playerIps.containsKey(pIp)) { int count = playerIps.get(pIp); if
			 * (count < Config.TOPZONE_DUALBOXES_ALLOWED) { playerIps.remove(pIp); playerIps.put(pIp, count + 1); canReward = true; } } else { canReward = true; playerIps.put(pIp, 1); } if (canReward) { addItem(player, Config.TOPZONE_REWARD_ID, Config.TOPZONE_REWARD_COUNT);
			 * player.sendMessage("You have received an award for his voice to the HOPZONE in the amount of " + Config.TOPZONE_REWARD_COUNT); } else { player.sendMessage("Already " + Config.TOPZONE_DUALBOXES_ALLOWED +
			 * " character(s) of your ip have been rewarded, so this character won't be rewarded."); } } playerIps.clear(); }
			 */
	...
	/*
	 * private static int getFirstPageRankVotes() { InputStreamReader isr = null; BufferedReader br = null; try { URLConnection con = new URL(page1Url).openConnection(); con.addRequestProperty("User-Agent", "Mozilla/4.76"); isr = new InputStreamReader(con.getInputStream()); br = new
	 * BufferedReader(isr); String line; int i = 0; while ((line = br.readLine()) != null) { if (line.contains("<td><div align=\"center\">" + firstPageRankNeeded + "</div></td>")) { i++; } if (line.contains("<td><div align=\"center\">") && (i == 1)) { i++; } if
	 * (line.contains("<td><div align=\"center\">") && (i == 2)) { i = 0; int votes = Integer.valueOf(line.split(">")[2].replace("</div", "")); return votes; } } br.close(); isr.close(); } catch (Exception e) { _log.warn("Topzone: Error while getting server vote count.", e); } return -1; }
	 */
	...
	private static int getVotes()
	{
		try
		{
			...
			/*
			 * _log.info("Votes are: " + voted); _log.info("Convert to int: " + Integer.parseInt(voted));
			 */
			return Integer.parseInt(voted);
		}
		catch (Exception e)
		{
			_log.warn("Mass Vote Reward for Topzone: Problem with getting votes from Topzone website: " + e.getMessage());
		}
		return -1;
	}
	...
	private static String getApiResponse(String endpoint)
	{
		...
		try
		{
			if (endpoint == null || endpoint.trim().isEmpty() || (!endpoint.startsWith("http://") && !endpoint.startsWith("https://")))
			{
				return "err";
			}
		...
		{
			_log.warn("Something went wrong in VoteBase::getApiResponse: " + e.getMessage());
			return "err";
		}
	}
