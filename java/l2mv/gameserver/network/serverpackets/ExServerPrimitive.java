...
public class ExServerPrimitive extends L2GameServerPacket
{
	...
	public void addCircle(Color color, int radius, int segments, int zOffset)
	{
		double angleStep = 2.0 * Math.PI / segments;
		int lastX = this._x + radius;
		int lastY = this._y;
		int z = this._z + zOffset;
		for (int i = 1; i <= segments; i++)
		{
			double angle = i * angleStep;
			int nextX = this._x + (int) (radius * Math.cos(angle));
			int nextY = this._y + (int) (radius * Math.sin(angle));
			this.addLine(color, lastX, lastY, z, nextX, nextY, z);
			lastX = nextX;
			lastY = nextY;
		}
	}
