		...
		try
		{
			...
			for (Player player : GameObjectsStorage.getAllPlayers())
			{
				...
				if (mult > 0)
				{
					...
					/*
					 * for(int i = 0; i < Config.L2_TOP_REWARD.length; i += 2) { if (Config.L2_TOP_REWARD[i] == -100) // PC Bang { player.addPcBangPoints((Config.L2_TOP_REWARD[i + 1] * mult), false); Log.add(player.getName() + " | " + player.getObjectId() + " | L2Top reward item ID | " +
					 * Config.L2_TOP_REWARD[i] + " | L2Top reward count | " + (Config.L2_TOP_REWARD[i + 1] * mult) + " |", "l2top"); } else if (Config.L2_TOP_REWARD[i] == -200) // Clan reputation { player.getClan().incReputation((Config.L2_TOP_REWARD[i + 1] * mult)); Log.add(player.getName() + " | "
					 * + player.getObjectId() + " | L2Top reward item ID | " + Config.L2_TOP_REWARD[i] + " | L2Top reward count | " + (Config.L2_TOP_REWARD[i + 1] * mult) + " |", "l2top"); } else if (Config.L2_TOP_REWARD[i] == -300) // Fame { player.setFame(player.getFame() + (Config.L2_TOP_REWARD[i
					 * + 1] * mult)); Log.add(player.getName() + " | " + player.getObjectId() + " | L2Top reward item ID | " + Config.L2_TOP_REWARD[i] + " | L2Top reward count | " + (Config.L2_TOP_REWARD[i + 1] * mult) + " |", "l2top"); } else { player.getInventory().addItem(Config.L2_TOP_REWARD[i],
					 * (Config.L2_TOP_REWARD[i + 1] * mult)); Log.add(player.getName() + " | " + player.getObjectId() + " | L2Top reward item ID | " + Config.L2_TOP_REWARD[i] + " | L2Top reward count | " + (Config.L2_TOP_REWARD[i + 1] * mult) + " |", "l2top"); } }
					 */
				}
			}
		}
