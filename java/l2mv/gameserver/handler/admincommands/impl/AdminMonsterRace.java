...
public class AdminMonsterRace implements IAdminCommandHandler
{
	...
	class RunRace extends RunnableImpl
	{
		...
		public void runImpl() throws Exception
		{
			...
			/*
			 * int[] speed = new int[8]; for(int i=0; i<8; i++) { for(int j=0; j<20; j++) { //_log.info.println("Adding "+speeds1[i][j] +" and "+ speeds2[i][j]); speed[i] += (speeds1[i][j]*1);// + (speeds2[i][j]*1); } _log.info.println("Total speed for "+(i+1)+" = "+speed[i]); }
			 */
