...
import premium.gameserver.model.TeleportLocation;
...
import premium.gameserver.model.actor.listener.NpcListenerList;
import premium.gameserver.model.actor.recorder.NpcStatsChangeRecorder;
...
import premium.gameserver.network.serverpackets.AcquireSkillDone;
import premium.gameserver.network.serverpackets.AcquireSkillList;
...
import premium.gameserver.network.serverpackets.ExChangeNpcState;
...
import premium.gameserver.taskmanager.DecayTaskManager;
...
import premium.gameserver.templates.npc.Faction;
	...
	public void showSkillList(Player player)
	{
		...
		if (!(getTemplate().canTeach(classId) || getTemplate().canTeach(classId.getParent(player.getSex()))))
		{
			...
			else
			{
				...
				StringBuilder sb = new StringBuilder();
				sb.append("<html><head><body>");
				sb.append(new CustomMessage("premium.gameserver.model.instances.L2NpcInstance.WrongTeacherClass", player));
				sb.append("</body></html>");
				html.setHtml(sb.toString());
				player.sendPacket(html);
			}
