		...
		else if (command.startsWith("SevenSigns"))
		{
			...
			switch (val)
			{
				...
				case 4: // Join a Cabal - SevenSigns 4 [0]1 x
					...
					{
						player.sendMessage(new CustomMessage("premium.gameserver.model.instances.L2SignsPriestInstance.AlreadyMember", player).addString(SevenSigns.getCabalName(cabal)));
						return;
					}
					if (player.getClassId().level() == 0)
					{
						player.sendMessage(new CustomMessage("premium.gameserver.model.instances.L2SignsPriestInstance.YouAreNewbie", player));
						break;
					}
					...
					else if (player.getClassId().level() >= 2)
					{
						if (Config.ALT_GAME_REQUIRE_CASTLE_DAWN)
						{
							if (getPlayerAllyHasCastle(player))
							{
								...
								{
									player.sendMessage(new CustomMessage("premium.gameserver.model.instances.L2SignsPriestInstance.CastleOwning", player));
									return;
								}
							}
							else
							...
							if (cabal == SevenSigns.CABAL_DAWN)
							{
								...
								if (!allowJoinDawn)
								{
									...
									{
										player.sendMessage(new CustomMessage("premium.gameserver.model.instances.L2SignsPriestInstance.CastleOwningCertificate", player));
									}
									else
									{
										player.sendMessage(new CustomMessage("premium.gameserver.model.instances.L2SignsPriestInstance.CastleOwningCertificate2", player));
									}
									return;
								}
							}
				...
				case 6: // Contribute Seal Stones - SevenSigns 6 x
					...
					else
					{
						...
						if (!stonesFound)
						{
							player.sendMessage(new CustomMessage("premium.gameserver.model.instances.L2SignsPriestInstance.DontHaveAnySSType", player));
							return;
						}
				...
				case 17: // Exchange Seal Stones for Ancient Adena (Type Choice) - SevenSigns 17 x
					...
					if (stoneType == 4)
					{
						...
						{
							player.sendMessage(new CustomMessage("premium.gameserver.model.instances.L2SignsPriestInstance.DontHaveAnySS", player));
						}
						break;
					}
				...
				case 18: // Exchange Seal Stones for Ancient Adena - SevenSigns 18 xxxx xxxxxx
					...
					{
						player.sendMessage(new CustomMessage("premium.gameserver.model.instances.L2SignsPriestInstance.DontHaveSSAmount", player));
					}
					break;
