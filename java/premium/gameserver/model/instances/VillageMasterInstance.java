...
import premium.gameserver.model.base.ClassType;
import premium.gameserver.model.base.PlayerClass;
...
import premium.gameserver.network.serverpackets.PledgeReceiveSubPledgeCreated;
...
import premium.gameserver.network.serverpackets.PledgeStatusChanged;
...
import premium.gameserver.utils.CertificationFunctions;
...
public final class VillageMasterInstance extends NpcInstance
{
		...
		else if (command.startsWith("Subclass"))
		{
			...
			switch (intVal)
			{
				case 1: // Returns a list of subs that can be taken (see case 4)
					...
					{
						player.sendMessage(new CustomMessage("premium.gameserver.model.instances.L2VillageMasterInstance.NoSubAtThisTime", player));
						return;
					}
					break;
				...
				case 7: // Отмена сабкласса - обработка выбора из case 6
					...
					{
						player.sendMessage(new CustomMessage("premium.gameserver.model.instances.L2VillageMasterInstance.SubclassCouldNotBeAdded", player));
						return;
					}
					break;
			}
	...
	public void setLeader(Player leader, String newLeader)
	{
		...
		if (member == null)
		{
			...
			// leader.sendMessage(new CustomMessage("premium.gameserver.model.instances.L2VillageMasterInstance.S1IsNotMemberOfTheClan", leader).addString(newLeader));
		...
		{
			leader.sendMessage(new CustomMessage("premium.gameserver.model.instances.L2VillageMasterInstance.CannotAssignUnitLeader", leader));
			return;
		}
	...
	{
		player.sendMessage(new CustomMessage("premium.gameserver.model.instances.L2VillageMasterInstance.ClanLeaderWillBeChangedFromS1ToS2", player).addString(clan.getLeaderName()).addString(newLeader.getName()));
		...
		/*
		 * if (clan.getLevel() >= CastleSiegeManager.getSiegeClanMinLevel()) { if (clan.getLeader() != null) { L2Player oldLeaderPlayer = clan.getLeader().getPlayer(); if (oldLeaderPlayer != null) SiegeUtils.removeSiegeSkills(oldLeaderPlayer); } L2Player newLeaderPlayer = newLeader.getPlayer(); if
		 * (newLeaderPlayer != null) SiegeUtils.addSiegeSkills(newLeaderPlayer); }
		 */
	...
	public static void createSubPledge(Player player, String clanName, int pledgeType, int minClanLvl, String leaderName)
	{
		...
		if (pledgeType != Clan.SUBUNIT_ACADEMY)
		{
			...
			if (subLeader == null)
			{
				player.sendMessage(new CustomMessage("premium.gameserver.model.instances.L2VillageMasterInstance.PlayerCantBeAssignedAsSubUnitLeader", player));
				return;
			}
			else if (subLeader.getLeaderOf() != Clan.SUBUNIT_NONE)
			{
				player.sendMessage(new CustomMessage("premium.gameserver.model.instances.L2VillageMasterInstance.ItCantBeSubUnitLeader", player));
				return;
			}
		}
	...
	public static void assignSubPledgeLeader(Player player, String clanName, String leaderName)
	{
		...
		if (clan == null)
		{
			player.sendMessage(new CustomMessage("premium.gameserver.model.instances.L2VillageMasterInstance.ClanDoesntExist", player));
			return;
		}
		...
		if (targetUnit == null)
		{
			player.sendMessage(new CustomMessage("premium.gameserver.model.instances.L2VillageMasterInstance.SubUnitNotFound", player));
			return;
		}
		SubUnit mainUnit = clan.getSubUnit(Clan.SUBUNIT_MAIN_CLAN);
	...
	public static boolean addNewSubclass(Player player, int classId)
	{
		...
		{
			player.sendMessage(new CustomMessage("premium.gameserver.model.instances.L2VillageMasterInstance.NoSubBeforeLevel", player).addNumber(Config.ALT_GAME_LEVEL_TO_GET_SUBCLASS));
			return false;
		}
		...
		if (!Config.ALT_GAME_SUBCLASS_WITHOUT_QUESTS && !playerClassList.isEmpty() && playerClassList.size() < 2 + Config.ALT_GAME_SUB_ADD)
		{
			if (player.isQuestCompleted("_234_FatesWhisper"))
			{
				if (player.getRace() == Race.kamael)
				{
					...
					{
						player.sendMessage(new CustomMessage("premium.gameserver.model.instances.L2VillageMasterInstance.QuestSeedsOfChaos", player));
						return false;
					}
				}
				else if (!player.isQuestCompleted("_235_MimirsElixir"))
				{
					player.sendMessage(new CustomMessage("premium.gameserver.model.instances.L2VillageMasterInstance.QuestMimirsElixir", player));
					return false;
				}
			}
			else
			{
				player.sendMessage(new CustomMessage("premium.gameserver.model.instances.L2VillageMasterInstance.QuestFatesWhisper", player));
				return false;
			}
		}
