...
import premium.gameserver.data.xml.holder.DressArmorHolder;
import premium.gameserver.instancemanager.CursedWeaponsManager;
import premium.gameserver.model.DressArmorData;
import premium.gameserver.model.Player;
import premium.gameserver.model.Zone;
import premium.gameserver.model.items.ItemInstance.ItemLocation;
import premium.gameserver.model.items.listeners.AccessoryListener;
import premium.gameserver.model.items.listeners.ArmorSetListener;
import premium.gameserver.model.items.listeners.BowListener;
import premium.gameserver.model.items.listeners.ItemAugmentationListener;
import premium.gameserver.model.items.listeners.ItemEnchantOptionsListener;
import premium.gameserver.model.items.listeners.ItemSkillsListener;
...
public class PcInventory extends Inventory
{
	...
	{
		ItemInstance item = super.addItem(itemId, count, owner, log);
		if (item != null)
		{
			premium.gameserver.inertiax.model.Inertia inertia = premium.gameserver.inertiax.controller.InertiaController.getInstance().getInertia(_owner);
			if (inertia != null)
			{
				inertia.onItemDrop(itemId, count);
			}
		}
		return item;
	}
	...
	public ItemInstance addItem(ItemInstance item, String owner, String log)
	{
		if (item != null)
		{
			...
			ItemInstance result = super.addItem(item, owner, log);
			if (result != null)
			{
				premium.gameserver.inertiax.model.Inertia inertia = premium.gameserver.inertiax.controller.InertiaController.getInstance().getInertia(_owner);
				if (inertia != null)
		...
		return super.addItem(item, owner, log);
	}
	...
	public boolean hasAllDressMeItemsEquipped()
	{
		...
		/*
		 * DressArmorData dress = DressArmorHolder.getInstance().getArmorByPartId(glovesItem.getVisualItemId() != 0 ? glovesItem.getVisualItemId() : feetItem.getVisualItemId()); if (dress == null) return false; if (chestItem.getVisualItemId() == 0) { chestItem.setVisualItemId(dress.getChest());
		 * chestItem.setJdbcState(JdbcEntityState.UPDATED); chestItem.update(); } if (legsItem.getVisualItemId() == 0) { legsItem.setVisualItemId(dress.getLegs()); legsItem.setJdbcState(JdbcEntityState.UPDATED); legsItem.update(); } if (glovesItem.getVisualItemId() == 0) {
		 * glovesItem.setVisualItemId(dress.getGloves()); glovesItem.setJdbcState(JdbcEntityState.UPDATED); glovesItem.update(); } if (feetItem.getVisualItemId() == 0) { feetItem.setVisualItemId(dress.getFeet()); feetItem.setJdbcState(JdbcEntityState.UPDATED); feetItem.update(); }
		 */
