package premium.gameserver.model.entity.SevenSignsFestival;
	...
	public void collapse()
	{
		...
		if (SevenSigns.getInstance().getCurrentPeriod() == SevenSigns.PERIOD_COMPETITION && getParty() != null)
		{
			...
			if (player.getInventory().destroyItem(bloodOfferings, "DarknessFestival"))
			{
				...
				sendCustomMessageToParticipants("premium.gameserver.model.entity.SevenSignsFestival.Ended");
				if (isHighestScore)
				{
			...
			{
				player.sendMessage(new CustomMessage("premium.gameserver.model.instances.L2FestivalGuideInstance.BloodOfferings", player));
			}
		}
