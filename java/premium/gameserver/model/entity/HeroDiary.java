	...
	public Map.Entry<String, String> toString(Player player)
	{
		...
		switch (_id)
		{
			case ACTION_RAID_KILLED:
				message = new CustomMessage("premium.gameserver.model.entity.Hero.RaidBossKilled", player).addString(HtmlUtils.htmlNpcName(_param));
				break;
			case ACTION_HERO_GAINED:
				message = new CustomMessage("premium.gameserver.model.entity.Hero.HeroGained", player);
				break;
			case ACTION_CASTLE_TAKEN:
				message = new CustomMessage("premium.gameserver.model.entity.Hero.CastleTaken", player).addString(HtmlUtils.htmlResidenceName(_param));
				break;
			default:
				return null;
		}
