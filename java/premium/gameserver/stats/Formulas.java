...
public class Formulas
{
	...
	public static AttackInfo calcPhysDam(Creature attacker, Creature target, Skill skill, boolean dual, boolean blow, boolean ss, boolean onCrit)
	{
		...
		if (skill != null)
		{
			...
			if (trait != null)
			{
				...
				/*
				 * else if (traitMul > 2.) // DS: нужны тесты traitMul = 2.; else if (traitMul < 0.05) traitMul = 0.05; power *= traitMul;
				 */
			}
	...
	public static boolean calcSkillSuccess(Env env, EffectTemplate et, int spiritshot)
	{
		...
		if (lvlDependMod != 0)
		{
			...
			/*
			 * final int delta = attackLevel - target.getLevel(); lvlDependMod = delta / 5; lvlDependMod = lvlDependMod * 5; if (lvlDependMod != delta) lvlDependMod = delta < 0 ? lvlDependMod - 5 : lvlDependMod + 5; env.value += lvlDependMod;
			 */
