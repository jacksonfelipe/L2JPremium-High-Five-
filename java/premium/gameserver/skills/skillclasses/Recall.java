...
public class Recall extends Skill
{
	...
	public boolean checkCondition(Creature activeChar, Creature target, boolean forceUse, boolean dontMove, boolean first)
	{
		...
		if (activeChar.isInZone(ZoneType.no_escape) || _townId > 0 && activeChar.getReflection() != null && activeChar.getReflection().getCoreLoc() != null)
		{
			if (activeChar.isPlayer())
			{
				activeChar.sendMessage(new CustomMessage("premium.gameserver.skills.skillclasses.Recall.Here", (Player) activeChar));
			}
			return false;
		}
	...
	public void useSkill(Creature activeChar, List<Creature> targets)
	{
		for (Creature target : targets)
		{
			if (target != null)
			{
				...
				{
					activeChar.sendMessage(new CustomMessage("premium.gameserver.skills.skillclasses.Recall.Festival", (Player) activeChar));
					continue;
				}
