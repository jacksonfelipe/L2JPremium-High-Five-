...
public class FishingChampionShipManager
{
	...
	public synchronized void newFish(Player pl, int lureId)
	{
		...
		if (_tmpPlayers.size() < 5)
		{
			for (Fisher fisher : _tmpPlayers)
			{
				if (fisher.getName().equalsIgnoreCase(pl.getName()))
				{
					if (fisher.getLength() < len)
					{
						...
						pl.sendMessage(new CustomMessage("premium.gameserver.instancemanager.games.FishingChampionShipManager.ResultImproveOn", pl));
						recalculateMinLength();
					}
					return;
				}
			...
			pl.sendMessage(new CustomMessage("premium.gameserver.instancemanager.games.FishingChampionShipManager.YouInAPrizeList", pl));
			recalculateMinLength();
		}
