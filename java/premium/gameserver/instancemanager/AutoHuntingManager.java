...
import premium.gameserver.randoms.CaptchaImage;
...
public class AutoHuntingManager
{
	...
	private void checkAndFixTable()
	{
		...
		try
		{
			...
			try (PreparedStatement ps = con.prepareStatement(
				...
				"  `reported_name` varchar(35) NOT NULL," +
				"  `reported_objectId` int(11) NOT NULL," +
				"  `reporter_name` varchar(35) NOT NULL," +
				"  `reporter_objectId` int(11) NOT NULL," +
			...
			try (PreparedStatement ps = con.prepareStatement("ALTER TABLE `bot_report` ADD COLUMN `reportType` VARCHAR(255) NOT NULL DEFAULT ''"))
			{
				ps.executeUpdate();
			}
			catch (Exception ignored) {}

			try (PreparedStatement ps = con.prepareStatement("ALTER TABLE `bot_report` ADD COLUMN `info` VARCHAR(255) NOT NULL DEFAULT ''"))
			{
				ps.executeUpdate();
			}
			catch (Exception ignored) {}
		...
		{
			_log.warn("AutoHuntingManager: Error while checking/fixing bot_report table: " + e.getMessage());
		}
		finally
		{
	...
	public synchronized void reportBot(Player reported, Player reporter, String typeOfReport, String moreInfo)
	{
		...
		{
			reporter.sendMessage(new CustomMessage("premium.gameserver.instancemanager.autohuntingmanager.message1", reporter));
			return;
		}
		...
		/*
		 * if (!reported.isInCombat() && !reported.isInPeaceZone()) { reporter.sendMessage(new CustomMessage("premium.gameserver.instancemanager.autohuntingmanager.message7", reporter)); return; }
		 */
		...
		{
			reporter.sendMessage(new CustomMessage("premium.gameserver.instancemanager.autohuntingmanager.message8", reporter));
			return;
		}
	...
	public int getPlayerReportsCount(Player reported)
	{
		...
		{
			_log.warn("Could not get player reports count: " + e.getMessage());
		}
		finally
		{
			DbUtils.closeQuietly(con, statement, rset);
	...
	public void savePlayerPunish(Player punished)
	{
		...
		{
			_log.warn("Could not save player punish: " + e.getMessage());
		}
		finally
		{
			DbUtils.closeQuietly(con, statement);
	...
	public boolean validateBot(Player reported, Player reporter)
	{
		...
		{
			reporter.sendMessage(new CustomMessage("premium.gameserver.instancemanager.autohuntingmanager.message3", reporter));
			return false;
		}
		/*
		...
		{
			reporter.sendMessage(new CustomMessage("premium.gameserver.instancemanager.autohuntingmanager.message4", reporter));
			return false;
		}
		if (reporter.getLevel() < Config.CAPTCHA_MIN_LEVEL)
		{
			reporter.sendMessage(new CustomMessage("premium.gameserver.instancemanager.autohuntingmanager.message6", reporter));
			return false;
		}
		// Same HWID, ip
		if (reporter.isDualbox(reported))
		{
			reporter.sendMessage(new CustomMessage("premium.gameserver.instancemanager.autohuntingmanager.message5", reporter));
			return false;
		}
	...
	private void restorePlayerBotPunishment(Player activeChar)
	{
		...
		{
			_log.warn("Could not restore player bot punishment: " + e.getMessage());
		}
		finally
		{
			DbUtils.closeQuietly(con, statement, rset);
