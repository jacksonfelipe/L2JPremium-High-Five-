 ...
 */
package premium.gameserver.masteriopack.rankpvpsystem;
...
import premium.gameserver.com.l2jserver.gameserver.masteriopack.imageconverter.ServerSideImage;
import premium.gameserver.kara.vote.StringUtil;

//import com.l2jserver.gameserver.premium.gameserver.masteriopack.imageconverter.ServerSideImage;
...
{
	private static final org.slf4j.Logger log = LoggerFactory.getLogger(RPSConfig.class);
	...
	public static void load()
	{
		...
		try
		{
			...
			if (DATABASE_CLEANER_REPEAT_TIME <= 0)
			{
				...
				log.warn("[DatabaseCleanerRepeatTime]: invalid config property -> \"" + DATABASE_CLEANER_REPEAT_TIME + "\"");
			}
			else
			{
			...
			if ((propertyValue != null) && (propertyValue.length() > 0))
			{
				...
				if (propertySplit.length > 0)
				{
					for (String value : propertySplit)
					{
						...
						{
							log.warn(StringUtil.concat("[ComboKillLocalAreaMessages]: invalid config property -> \"", value, "\""));
						}
						else
						{
							try
							...
							catch (NumberFormatException nfe)
							{
								if (!value.isEmpty())
								{
									log.warn(StringUtil.concat("[ComboKillLocalAreaMessages]: invalid config property -> \"", valueSplit[0], "\"", valueSplit[1]));
								}
							}
						}
					}
			...
			if ((propertyValue != null) && (propertyValue.length() > 0))
			{
				...
				if (propertySplit.length > 0)
				{
					for (String value : propertySplit)
					{
						...
						if (valueSplit.length != 2)
						{
							log.warn(StringUtil.concat("[ComboKillGlobalAreaMessages]: invalid config property -> \"", value, "\""));
						}
						else
						{
							try
							...
							catch (NumberFormatException nfe)
							{
								if (!value.isEmpty())
								{
									log.warn(StringUtil.concat("[ComboKillGlobalAreaMessages]: invalid config property -> \"", valueSplit[0], "\"", valueSplit[1]));
								}
							}
						}
					}
			...
			if ((propertyValue != null) && (propertyValue.length() > 0))
			{
				...
				if (propertySplit.length > 0)
				{
					for (String value : propertySplit)
					{
						...
						if (valueSplit.length != 2)
						{
							log.warn(StringUtil.concat("[ComboKillRankPointsRatio]: invalid config property -> \"", value, "\""));
						}
						else
						{
							try
							...
							catch (NumberFormatException nfe)
							{
								if (!value.isEmpty())
								{
									log.warn(StringUtil.concat("[ComboKillRankPointsRatio]: invalid config property -> \"", valueSplit[0], "\"", valueSplit[1]));
								}
							}
						}
					}
			...
			if ((propertyValue != null) && (propertyValue.length() > 0))
			{
				...
				if (propertySplit.length > 0)
				{
					for (String value : propertySplit)
					{
						...
						if (valueSplit.length != 2)
						{
							log.warn(StringUtil.concat("[RankPointsBonusZonesIds]: invalid config property -> \"", value, "\""));
						}
						else
						{
							try
							...
							catch (NumberFormatException nfe)
							{
								if (!value.isEmpty())
								{
									log.warn(StringUtil.concat("[RankPointsBonusZonesIds]: invalid config property -> \"", valueSplit[0], "\"", valueSplit[1]));
								}
							}
						}
					}
			...
			{
				log.warn(StringUtil.concat("[PvpTableUpdateInterval]: invalid config property -> \"", Long.toString(PVP_TABLE_UPDATE_INTERVAL), "\""));
			...
			if ((propertyValue != null) && (propertyValue.length() > 0))
			{
				...
				if (propertySplit.length > 0)
				{
					for (String value : propertySplit)
					{
						...
						{
							log.warn(StringUtil.concat("[TopTableUpdateTimes]: invalid config property -> \"", value, "\""));
						}
						else
						{
							try
							{
								...
								if (m_s.length() != 2)
								{
									log.warn(StringUtil.concat("[TopTableUpdateTimes]: invalid config property -> \"", value, "\" minutes format incorrect [hh:mm]"));
									break;
								}
			...
			log.info(" - Rank Pvp System Config initialization complete.");
		}
		catch (Exception e)
		{
			log.warn("Config: " + e.getMessage());
