...
{
	private static final org.slf4j.Logger _log = LoggerFactory.getLogger(TopTable.class);
	...
	private void load()
	{
		...
		_log.info(" - TopTable: Data " + info + ". " + _topKillsTable.size() + " killers and " + _topGatherersTable.size() + " gatherers in " + (endTime - startTime) + " ms.");
		_log.info(" - TopTable: Next update on " + RPSUtil.dateToString(times[2]) + " at " + RPSUtil.timeToString(times[2]) + ".");
	}
	...
	private static class TopTableSchedule implements Runnable
	{
		...
		public void run()
		{
			...
			{
				_log.info("TopTable: Data updated in " + (Calendar.getInstance().getTimeInMillis() - times[0]) + " ms <<< Next update on " + RPSUtil.dateToString(times[2]) + " at " + RPSUtil.timeToString(times[2]));
				
				ThreadPoolManager.getInstance().schedule(new TopTableSchedule(), times[1]);
			}
			else
			{
				_log.info("TopTable: Data update failed! <<< Next try for 5 minutes.");
