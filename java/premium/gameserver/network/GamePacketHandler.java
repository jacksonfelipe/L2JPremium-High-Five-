...
import premium.gameserver.network.clientpackets.*;
...
public final class GamePacketHandler implements IPacketHandler<GameClient>, IClientFactory<GameClient>, IMMOExecutor<GameClient>
{
	...
	public ReceivablePacket<GameClient> handlePacket(ByteBuffer buf, GameClient client)
	{
		...
		/*
		 * // Synerge - Support for real character block. If the character is blocked, we wont be able to do anything until its unblocked. Certain packets do not have this condition if (msg != null && !msg.canBeUsedWhileBlocked() && client.getActiveChar() != null &&
		 * !client.getActiveChar().isInObserverMode() && client.getActiveChar().isFullBlocked()) return null;
		 */
