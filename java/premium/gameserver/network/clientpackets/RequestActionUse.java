...
import premium.gameserver.network.serverpackets.ExAirShipTeleportList;
import premium.gameserver.network.serverpackets.ExAskCoupleAction;
import premium.gameserver.network.serverpackets.MyTargetSelected;
import premium.gameserver.network.serverpackets.PrivateStoreManageListBuy;
import premium.gameserver.network.serverpackets.PrivateStoreManageListSell;
import premium.gameserver.network.serverpackets.RecipeShopManageList;
...
public class RequestActionUse extends L2GameClientPacket
{
	...
	/*
	 * type: 0 - action 1 - pet action 2 - pet skill 3 - social 4 - couple social transform: 0 for any permitted 1 allowed for some 2 prohibited for all
	 */
	...
	protected void runImpl()
	{
		...
		switch (action.id)
		{
			...
			/*
			 * case 65: // Bot Report Button if (activeChar.getTarget() == null || !activeChar.getTarget().isPlayer()) { activeChar.sendMessage("You need to target Player first!"); return; } CaptchaHandler.tryReportPlayer(activeChar, activeChar.getTarget().getPlayer()); break;
			 */
