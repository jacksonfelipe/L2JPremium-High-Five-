...
import premium.gameserver.network.serverpackets.ExGMViewQuestItemList;
import premium.gameserver.network.serverpackets.ExUseSharedGroupItem;
import premium.gameserver.network.serverpackets.GMHennaInfo;
import premium.gameserver.network.serverpackets.GMViewItemList;
...
public class UseItem extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		/*
		 * if (activeChar._event != null && activeChar._event.getName().equalsIgnoreCase("Capture The Flag") && (itemId == 13560 || itemId == 13561)) { activeChar.sendMessage(new CustomMessage("premium.gameserver.network.l2.c2s.UseItem.NotUseIsEvents", activeChar)); return; }
		 */
		...
		{
			activeChar.sendMessage(new CustomMessage("premium.gameserver.network.l2.c2s.UseItem.NotUseIsFlag", activeChar));
			return;
		}
		
		/*
		 * if (ArrayUtils.contains(Config.ITEM_USE_LIST_ID, itemId) && !Config.ITEM_USE_IS_EVENTS && Events.onAction(activeChar, activeChar, true)) { activeChar.sendMessage(new CustomMessage("premium.gameserver.network.l2.c2s.UseItem.NotUseIsEvents", activeChar)); return; }
		 */
