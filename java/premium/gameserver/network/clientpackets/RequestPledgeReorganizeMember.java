...
public class RequestPledgeReorganizeMember extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		{
			activeChar.sendMessage(new CustomMessage("premium.gameserver.clientpackets.RequestPledgeReorganizeMember.ChangeAffiliations", activeChar));
			activeChar.sendActionFailed();
			return;
		}
		...
		if (subject == null)
		{
			activeChar.sendMessage(new CustomMessage("premium.gameserver.clientpackets.RequestPledgeReorganizeMember.NotInYourClan", activeChar));
			activeChar.sendActionFailed();
			return;
		}
		...
		{
			activeChar.sendMessage(new CustomMessage("premium.gameserver.clientpackets.RequestPledgeReorganizeMember.AlreadyInThatCombatUnit", activeChar));
			activeChar.sendActionFailed();
			return;
		}
		...
		{
			activeChar.sendMessage(new CustomMessage("premium.gameserver.clientpackets.RequestPledgeReorganizeMember.NoSuchCombatUnit", activeChar));
			activeChar.sendActionFailed();
			return;
		}
		...
		if (this._replace > 0)
		{
			...
			if (replacement == null)
			{
				activeChar.sendMessage(new CustomMessage("premium.gameserver.clientpackets.RequestPledgeReorganizeMember.CharacterNotBelongClan", activeChar));
				activeChar.sendActionFailed();
				return;
			}
			if (replacement.getPledgeType() != this._targetUnit)
			{
				activeChar.sendMessage(new CustomMessage("premium.gameserver.clientpackets.RequestPledgeReorganizeMember.CharacterNotBelongCombatUnit", activeChar));
				activeChar.sendActionFailed();
				return;
			}
			if (replacement.isSubLeader() != 0)
			{
				activeChar.sendMessage(new CustomMessage("premium.gameserver.clientpackets.RequestPledgeReorganizeMember.CharacterLeaderAnotherCombatUnit", activeChar));
				activeChar.sendActionFailed();
				return;
			}
		}
		else
		{
			...
			{
				activeChar.sendMessage(new CustomMessage("premium.gameserver.clientpackets.RequestPledgeReorganizeMember.MemberLeaderAnotherUnit", activeChar));
				activeChar.sendActionFailed();
				return;
			}
