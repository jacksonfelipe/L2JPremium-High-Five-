...
import premium.gameserver.network.serverpackets.PledgeReceiveMemberInfo;
...
public class RequestPledgeSetAcademyMaster extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if ((activeChar.getClanPrivileges() & Clan.CP_CL_APPRENTICE) == Clan.CP_CL_APPRENTICE)
		{
			...
			if (sponsor != null && apprentice != null)
			{
				...
				if (this._mode == 1)
				{
					...
					{
						activeChar.sendMessage(new CustomMessage("premium.gameserver.clientpackets.RequestOustAlly.MemberAlreadyHasApprentice", activeChar));
						return;
					}
					if (apprentice.hasSponsor())
					{
						activeChar.sendMessage(new CustomMessage("premium.gameserver.clientpackets.RequestOustAlly.ApprenticeAlreadyHasSponsor", activeChar));
						return;
					}
				...
				else
				{
					if (!sponsor.hasApprentice())
					{
						activeChar.sendMessage(new CustomMessage("premium.gameserver.clientpackets.RequestOustAlly.MemberHasNoApprentice", activeChar));
						return;
					}
		...
		{
			activeChar.sendMessage(new CustomMessage("premium.gameserver.clientpackets.RequestOustAlly.NoMasterRights", activeChar));
		}
	}
}