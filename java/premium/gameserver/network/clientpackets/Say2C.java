...
import premium.gameserver.model.entity.forum.ShoutboxHandler;
...
public class Say2C extends L2GameClientPacket
{
	...
	protected void readImpl()
	{
		...
		this._type = premium.commons.lang.ArrayUtils.valid(ChatType.VALUES, this.readD());
	...
	protected void runImpl()
	{
		...
		/*
		 * else if (_text.startsWith(".") && _text.endsWith("hellbound") && Config.NOT_USE_USER_VOICED) // If Voice commands are not available, can only handle .hellbound { String fullcmd = _text.substring(1).trim(); String command = fullcmd.split("\\s+")[0]; String args =
		 * fullcmd.substring(command.length()).trim(); if (command.length() > 0) { // then check for VoicedCommands IVoicedCommandHandler vch = VoicedCommandHandler.getInstance().getVoicedCommandHandler(command); if (vch != null) { vch.useVoicedCommand(command, activeChar, args); return; } }
		 * activeChar.sendMessage(new CustomMessage("common.command404", activeChar)); return; }
		 */
