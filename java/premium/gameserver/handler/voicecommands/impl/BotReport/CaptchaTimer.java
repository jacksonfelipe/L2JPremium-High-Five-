package premium.gameserver.handler.voicecommands.impl.BotReport;
...
/**
 * Class handling Timer which calculates if Targets solved Captcha in Time It is also taking care of failedBotReporters, List of AccountNames, IPs and HWIDs of the players that reported Bot
 */
public class CaptchaTimer
{
	...
	/**
	 * Adding new Bot Reporter to the List If he already exists, just changing time of Last Report Otherwise Account Name, IP, HWID and current time are saved.
	...
	/**
	 * Removing Bot Reporter from the List He will now be able to Report Players
	...
	/**
	 * Checking If Player can report Bot now. Account Name, IP and HWID are being Checked
	...
	/**
	 * Creating new CaptchaEvent and adding it to the List CaptchaEventThread will now check, if player solved Captcha in time
	...
	/**
	 * Removing CaptchaEvent from the List Captcha Thread will no longer check this CaptchaEvent
	...
	/**
	 * Getting CaptchaEvent by Name of the Player It's checking only Names of Players that are solving Captchas, not reporters Returning NULL if CaptchEvent wasn't found
	...
	/**
	 * Getting Iteration of captchaEventList It gives all Captcha Events that Players are currently Solving
