...
public class AdminCheckBot implements IAdminCommandHandler
{
	...
	public boolean useAdminCommand(Enum comm, String[] wordList, String fullString, Player activeChar)
	{
		...
		{
			activeChar.sendMessage(new CustomMessage("premium.gameserver.handler.admincommands.impl.admincheckbot.notenabled", activeChar));
			return false;
		}
		...
		switch (command)
		{
			...
			case admin_punish_bot:
			{
				if (wordList != null)
				{
					...
					if (characterName != null)
					{
						if (wordList[2].equalsIgnoreCase("CHATBAN"))
						{
							...
							if (punishTime != 0)
							{
								...
								activeChar.sendMessage(new CustomMessage("premium.gameserver.handler.admincommands.impl.admincheckbot.you_chatbanned", activeChar, characterName, punishTime));
								introduceNewPunishedBotAndClear(characterName);
							}
							else
							{
								activeChar.sendMessage(new CustomMessage("premium.gameserver.handler.admincommands.impl.admincheckbot.error", activeChar));
							}
						}
						else if (wordList[2].equalsIgnoreCase("KICK"))
						{
							...
							activeChar.sendMessage(new CustomMessage("premium.gameserver.handler.admincommands.impl.admincheckbot.kicked", activeChar, characterName));
							introduceNewPunishedBotAndClear(characterName);
						}
						else if (wordList[2].equalsIgnoreCase("BANCHAR"))
						{
							...
							if (punishTime != 0)
							{
								...
								activeChar.sendMessage(new CustomMessage("premium.gameserver.handler.admincommands.impl.admincheckbot.banned", activeChar, characterName, punishTime));
								final Player plr = World.getPlayer(characterName);
								if (plr != null)
								{
									plr.kick();
						...
						else if (wordList[2].equalsIgnoreCase("BANACC"))
						{
							...
							if (accountName != null)
							{
								...
								if (client != null)
								{
									...
									{
										player.kick();
										activeChar.sendMessage(new CustomMessage("premium.gameserver.handler.admincommands.impl.admincheckbot.banned1", activeChar, characterName, accountName));
										introduceNewPunishedBotAndClear(characterName);
									}
								}
							}
					...
					{
						activeChar.sendMessage(new CustomMessage("premium.gameserver.handler.admincommands.impl.admincheckbot.doesnotexist", activeChar));
					}
				}
			}
		}
