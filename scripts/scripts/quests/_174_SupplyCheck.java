...
public class _174_SupplyCheck extends Quest implements ScriptFile
{
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if (npcId == Marcela)
		{
			...
			/*
			 * { st.setCond(5); st.takeItems(GroceryStoreManifest, -1); htmltext = "zerstorer_morsell_q0174_08.htm"; } else if (cond == 5) htmltext = "zerstorer_morsell_q0174_09.htm"; else if (cond == 6) { st.setCond(7); st.takeItems(WeaponShopManifest, -1); st.giveItems(SupplyReport, 1); htmltext =
			 * "zerstorer_morsell_q0174_10.htm"; } else if (cond == 7) htmltext = "zerstorer_morsell_q0174_11.htm"; else if (cond == 8) { if (st.getPlayer().getClassId().isMage() && !st.getPlayer().getClassId().equalsOrChildOf(ClassId.orcMage)) { st.giveItems(LeatherTunic, 1);
			 * st.giveItems(LeatherStockings, 1); } else { st.giveItems(WoodenBreastplate, 1); st.giveItems(WoodenGaiters, 1); } st.giveItems(WoodenHelmet, 1); st.giveItems(LeatherShoes, 1); st.giveItems(Gloves, 1); st.giveItems(ADENA_ID, 2466, true); st.getPlayer().addExpAndSp(5672, 446, false,
			 * false); if (st.getPlayer().getClassId().getLevel() == 1 && !st.getPlayer().getVarB("ng1")) st.getPlayer().sendPacket(new ExShowScreenMessage("  Delivery duty complete.\nGo find the Newbie Guide.", 5000, ScreenMessageAlign.TOP_CENTER, true)); st.exitCurrentQuest(false); htmltext =
			 * "zerstorer_morsell_q0174_12.htm"; }
			 */
		}
		...
		/*
		 * else if (npcId == Erinu) if (cond < 5) htmltext = "subelder_casca_q0174_01.htm"; else if (cond == 5) { st.setCond(6); st.giveItems(WeaponShopManifest, 1); htmltext = "trader_erinu_q0174_02.htm"; } else htmltext = "subelder_casca_q0174_03.htm"; else if (npcId == Casca) if (cond < 7)
		 * htmltext = "subelder_casca_q0174_01.htm"; else if (cond == 7) { st.setCond(8); st.takeItems(SupplyReport, -1); htmltext = "subelder_casca_q0174_02.htm"; } else htmltext = "subelder_casca_q0174_03.htm";
		 */
		return htmltext;
	}
}