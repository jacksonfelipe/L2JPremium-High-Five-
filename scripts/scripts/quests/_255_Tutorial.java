...
import premium.gameserver.model.entity.ChangeLogManager;
...
public class _255_Tutorial extends Quest implements ScriptFile, OnPlayerEnterListener
{
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		else if (event.startsWith("TryPass"))
		{
			...
			if (correct)
			{
				...
				{
					/*
					 * if (player.getClan() == null && player.getNetConnection().getBonus() < 1) { player.sendPacket(new NpcHtmlMessage(5).setFile("advertise.htm").replace("%playername%", player.getName())); }
					 */
	...
	/**
	 * If {@link #canChangeClass(premium.gameserver.model.Player, int) canChangeClass}, showing Tutorial Page with next Classes that player can advance to
	 * @param st
	 * @return
	 */
	...
	{
		/*
		 * if (player.getLevel() < 6) player.addListener(_tutorialShowListener);
		 */
	}
	
	/*
	 * public class TutorialShowListener implements OnCurrentHpDamageListener {
	 * @Override public void onCurrentHpDamage(Creature actor, double damage, Creature attacker, Skill skill) { Player player = actor.getPlayer(); if (player.getCurrentHpPercents() < 25) { player.removeListener(_tutorialShowListener); Quest q = QuestManager.getQuest(255); if (q != null)
	 * player.processQuestEvent(q.getName(), "CE45", null); } else if (player.getLevel() > 5) player.removeListener(_tutorialShowListener); } }
	 */
