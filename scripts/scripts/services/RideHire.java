...
public class RideHire extends Functions
{
	...
	public void ride(String[] args)
	{
		...
		/*
		 * if ((npc_id == PetDataTable.WYVERN_ID || npc_id == PetDataTable.STRIDER_WIND_ID) && !SiegeUtils.getCanRide()) { show(ru ? "Прокат виверн/страйдеров не работает во время осады." : "Can't ride wyvern/strider while Siege in progress.", player, npc); return; }
		 */
