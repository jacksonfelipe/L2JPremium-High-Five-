...
import premium.gameserver.randoms.GeoLocation;
...
public class CommunityBoard implements ScriptFile, ICommunityBoardHandler
{
	...
	public void onBypassCommand(Player player, String bypass)
	{
		...
		if ("bbshome".equals(cmd))
		{
			...
			if (dafault.equals(cmd))
			{
				...
				html = html.replace("<?geolocation?>", GeoLocation.getCountryCode(player) + "/" + GeoLocation.getCity(player));
				html = html.replace("<?server_uptime?>", String.valueOf(uptime()));
				// html = html.replace("%PlayerImage%", "%image:" + player.getRace() + ".png%");
				/*
				 * @claww - is working but useless.... try { if (!FileUtils.readFileToString(new File("/lucia/gameserver/data/html-en/scripts/services/communityPVP/pages/news.txt"), "UTF-8").trim().isEmpty()) { html = html.replace("<?news?>", String.valueOf(FileUtils.readFileToString(new
				 * File("/lucia/gameserver/data/html-en/scripts/services/communityPVP/pages/news.txt"), "UTF-8"))); } else { html = html.replace("<?news?>", String.valueOf("No news yet")); } } catch (IOException e) { _log.warn("Error while update <?news?>"); } try { if
				 * (!FileUtils.readFileToString(new File("/lucia/gameserver/data/html-en/scripts/services/communityPVP/pages/events.txt"), "UTF-8").trim().isEmpty()) { html = html.replace("<?gm_event?>", String.valueOf(FileUtils.readFileToString(new
				 * File("/lucia/gameserver/data/html-en/scripts/services/communityPVP/pages/events.txt"), "UTF-8"))); } else { html = html.replace("<?gm_event?>", String.valueOf("No GM events yet")); } } catch (IOException e) { _log.warn("Error while update <?news?>"); }
				 */
	 ...
	 * @return
	 */
	public String online(boolean off)
	{
		int i = 0;
		int j = 0;
	...
	public String getTimeInServer(Player player)
	{
