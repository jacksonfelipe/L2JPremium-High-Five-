	...
	private void loadRankings()
	{
		...
		try (Connection con = DatabaseFactory.getInstance().getConnection())
		{
			for (RankingEnum type : RankingEnum.values())
			{
				...
				{
					try (
						PreparedStatement statement = con.prepareStatement("SELECT obj_Id,char_name,class_id,clanid,it.count FROM characters AS c LEFT JOIN character_subclasses AS cs ON (c.obj_Id=cs.char_obj_id) JOIN items AS it ON (c.obj_Id=it.owner_id) WHERE cs.isBase=1 AND it.item_id=57 AND accesslevel = 0 ORDER BY it.count DESC");
				...
				{
					try (
						PreparedStatement statement = con.prepareStatement("SELECT obj_Id,char_name,class_id,clanid," + type.getDbName() + " FROM characters AS c LEFT JOIN character_subclasses AS cs ON (c.obj_Id=cs.char_obj_id) WHERE cs.isBase=1 AND accesslevel = 0 AND " + type.getDbName() + " > 0 ORDER BY " + type.getDbName() + " DESC");
						ResultSet rset = statement.executeQuery())
					{
						while (rset.next())
						{
