...
import premium.gameserver.instancemanager.UnderGroundColliseumManager;
...
public class UndergroundColiseumInstance extends NpcInstance
{
	...
	public void onBypassFeedback(Player player, String command)
	{
		...
		if (actualCommand.startsWith("register"))
		{
			...
			/*
			 * TODO раскомментировать if(player.getParty().size() < 7) { showChatWindow(player, 3); return; }
			 */
			if (st.hasMoreTokens())
			{
		...
		else if (actualCommand.startsWith("winner"))
		{
			...
			NpcHtmlMessage html;
			/*
			 * if(UnderGroundColliseumManager.getInstance().getColiseumByLevelLimit(getMaxLevel()).getPreviusWinners() != null) { filename = "Coliseum/"+ 7 + "htm"; html = new NpcHtmlMessage(player, this, filename, 7); html.replace("winner",
