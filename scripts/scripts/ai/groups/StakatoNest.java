...
public class StakatoNest extends Fighter
{
	...
	protected void onEvtDead(Creature killer)
	{
		...
		switch (actor.getNpcId())
		{
			...
			case FEMALE_SPIKED_STAKATO:
				...
				break;
			/*
			 * case CANNIBALISTIC_STAKATO_CHIEF: if (killer.isPlayer()) if (killer.getPlayer().getParty() != null) { List<L2Player> party = killer.getPlayer().getParty().getMembers(); for (L2Player member : party) giveCocoon(member); } else giveCocoon(killer.getPlayer()); break;
			 */
			case QUEEN_SHYEED:
				if (_debuffed)
				{
	...
	public MinionInstance getAliveMinion(NpcInstance npc)
	{
	...
	public void spawnMonster(NpcInstance actor, Creature killer, int mobId)
	{
		try
		{
